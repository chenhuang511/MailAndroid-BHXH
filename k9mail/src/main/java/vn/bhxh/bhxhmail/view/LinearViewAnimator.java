...
public class LinearViewAnimator extends ViewAnimator {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, vn.bhxh.bhxhmail.R.styleable.LinearViewAnimator);

        int resource = a.getResourceId(vn.bhxh.bhxhmail.R.styleable.LinearViewAnimator_downInAnimation, 0);
        if (resource > 0) {
            setDownInAnimation(context, resource);
        }

        resource = a.getResourceId(vn.bhxh.bhxhmail.R.styleable.LinearViewAnimator_downOutAnimation, 0);
        if (resource > 0) {
            setDownOutAnimation(context, resource);
        }

        resource = a.getResourceId(vn.bhxh.bhxhmail.R.styleable.LinearViewAnimator_upInAnimation, 0);
        if (resource > 0) {
            setUpInAnimation(context, resource);
        }

        resource = a.getResourceId(vn.bhxh.bhxhmail.R.styleable.LinearViewAnimator_upOutAnimation, 0);
        if (resource > 0) {
