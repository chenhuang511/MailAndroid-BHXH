    ...
    public String getOperation(Context context) {
        ...
        if (mLoadingAccountDescription  != null
            ...
            progress = (mFolderTotal > 0 ?
                        context.getString(vn.bhxh.bhxhmail.R.string.folder_progress, mFolderCompleted, mFolderTotal) : "");
            ...
            if (mLoadingFolderName != null || mLoadingHeaderFolderName != null) {
                ...
                if ((mAccount != null) && (mAccount.getInboxFolderName() != null) && mAccount.getInboxFolderName().equalsIgnoreCase(displayName)) {
                    displayName = context.getString(vn.bhxh.bhxhmail.R.string.special_mailbox_name_inbox);
                } else if ((mAccount != null) && mAccount.getOutboxFolderName().equals(displayName)) {
                    displayName = context.getString(vn.bhxh.bhxhmail.R.string.special_mailbox_name_outbox);
                }

                if (mLoadingHeaderFolderName != null) {

                    operation = context.getString(vn.bhxh.bhxhmail.R.string.status_loading_account_folder_headers, mLoadingAccountDescription, displayName, progress);
                } else {
                    operation = context.getString(vn.bhxh.bhxhmail.R.string.status_loading_account_folder, mLoadingAccountDescription, displayName, progress);
                }
            }

            else if (mSendingAccountDescription != null) {
                operation = context.getString(vn.bhxh.bhxhmail.R.string.status_sending_account, mSendingAccountDescription, progress);
            } else if (mProcessingAccountDescription != null) {
                operation = context.getString(vn.bhxh.bhxhmail.R.string.status_processing_account, mProcessingAccountDescription,
            ...
            if (nextPollTime != -1) {
                operation = context.getString(vn.bhxh.bhxhmail.R.string.status_next_poll,
                        DateUtils.getRelativeTimeSpanString(nextPollTime, System.currentTimeMillis(),
                                DateUtils.MINUTE_IN_MILLIS, 0));
            } else if (MailService.isSyncDisabled()) {
                operation = context.getString(vn.bhxh.bhxhmail.R.string.status_syncing_off);
            } else {
                operation = "";
            }
        }
