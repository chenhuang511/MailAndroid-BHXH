...
import vn.bhxh.bhxhmail.mail.AuthenticationFailedException;
import vn.bhxh.bhxhmail.mail.CertificateValidationException;
import vn.bhxh.bhxhmail.mail.MessagingException;
import vn.bhxh.bhxhmail.mail.Store;
import vn.bhxh.bhxhmail.mail.Transport;
import vn.bhxh.bhxhmail.mail.filter.Hex;
...
public class AccountSetupCheckSettings extends K9Activity implements OnClickListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(vn.bhxh.bhxhmail.R.layout.account_setup_check_settings);
        mMessageView = (TextView)findViewById(vn.bhxh.bhxhmail.R.id.message);
        mProgressBar = (ProgressBar)findViewById(vn.bhxh.bhxhmail.R.id.progress);
        findViewById(vn.bhxh.bhxhmail.R.id.cancel).setOnClickListener(this);

        setMessage(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_retr_info_msg);
    ...
    private void handleCertificateValidationException(CertificateValidationException cve) {
        ...
        if (chain != null) {
            acceptKeyDialog(
                    vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_certificate_message_fmt,
                    cve);
        } else {
            showErrorDialog(
                    vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_server_message_fmt,
...
//                .show();
    ...
    private void onCancel() {
        mCanceled = true;
        setMessage(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_canceling_msg);
    }

    public void onClick(View v) {
        switch (v.getId()) {
        case vn.bhxh.bhxhmail.R.id.cancel:
            onCancel();
            break;
        }
    }
    ...
    private void showErrorDialog(final int msgResId, final Object... args) {
        mHandler.post(new Runnable() {
            public void run() {
                showDialogFragment(vn.bhxh.bhxhmail.R.id.dialog_account_setup_error, getString(msgResId, args));
            }
        });
    }
    ...
    private void showDialogFragment(int dialogId, String customMessage) {
        ...
        switch (dialogId) {
            case vn.bhxh.bhxhmail.R.id.dialog_account_setup_error: {
                fragment = ConfirmationDialogFragment.newInstance(dialogId,
                        getString(vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_title),
                        customMessage,
                        getString(vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_edit_details_action),
                        getString(vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_continue_action)
                );
                break;
            }
            default: {
    ...
    private String errorMessageForCertificateException(CertificateValidationException e) {
        switch (e.getReason()) {
            case Expired: return getString(vn.bhxh.bhxhmail.R.string.client_certificate_expired, e.getAlias(), e.getMessage());
            case MissingCapability: return getString(vn.bhxh.bhxhmail.R.string.auth_external_error);
            case RetrievalFailure: return getString(vn.bhxh.bhxhmail.R.string.client_certificate_retrieval_failure, e.getAlias());
    ...
    private class CheckAccountTask extends AsyncTask<CheckDirection, Integer, Void> {
        ...
        protected Void doInBackground(CheckDirection... params) {
            ...
            try {
                ...
                showErrorDialog(
                        vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_auth_message_fmt,
                ...
                showErrorDialog(vn.bhxh.bhxhmail.R.string.account_setup_failed_dlg_server_message_fmt, message);
            }
            return null;
        }
        ...
        private void checkOutgoing() throws MessagingException {
            if (!(account.getRemoteStore() instanceof WebDavStore)) {
                publishProgress(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_check_outgoing_msg);
            }
        ...
        private void checkIncoming() throws MessagingException {
            ...
            if (store instanceof WebDavStore) {
                publishProgress(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_authenticate);
            } else {
                publishProgress(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_check_incoming_msg);
            }
            store.checkSettings();

            if (store instanceof WebDavStore) {
                publishProgress(vn.bhxh.bhxhmail.R.string.account_setup_check_settings_fetch);
            }
