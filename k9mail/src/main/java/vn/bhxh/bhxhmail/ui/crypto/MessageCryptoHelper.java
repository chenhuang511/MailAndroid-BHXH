...
import vn.bhxh.bhxhmail.mailstore.MessageHelper;
import vn.bhxh.bhxhmail.mailstore.MimePartStreamParser;
import vn.bhxh.bhxhmail.mailstore.util.FileFactory;
    ...
    private void processFoundSignedParts(List<Part> foundParts) {
        for (Part part : foundParts) {
            if (!MessageHelper.isCompletePartAvailable(part)) {
                ...
                addErrorAnnotation(part, CryptoResultAnnotation.CryptoError.OPENPGP_SIGNED_BUT_INCOMPLETE, replacementPart);
                continue;
            }
            if (MessageDecryptVerifier.isPgpMimeEncryptedOrSignedPart(part)) {
            ...
            MimeBodyPart replacementPart = getMultipartSignedContentPartIfAvailable(part);
            addErrorAnnotation(part, CryptoResultAnnotation.CryptoError.SIGNED_BUT_UNSUPPORTED, replacementPart);
        }
    }
