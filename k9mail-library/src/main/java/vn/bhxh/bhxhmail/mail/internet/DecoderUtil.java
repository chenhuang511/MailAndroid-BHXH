...
class DecoderUtil {
    ...
    private static String decodeEncodedWord(String body, int begin, int end, Message message) {
        ...
        if (encodedText.isEmpty()) {
            Log.w(K9MailLib.LOG_TAG, "Missing encoded text in encoded word: '" + body.substring(begin, end) + "'");
            return null;
        }
        ...
        if (encoding.equalsIgnoreCase("Q")) {
            ...
            Log.w(K9MailLib.LOG_TAG, "Warning: Unknown encoding in encoded word '" + body.substring(begin, end) + "'");
            return null;
        }
    }
}
